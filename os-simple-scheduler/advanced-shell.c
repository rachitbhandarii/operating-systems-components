...
void signal_handler(int signum) {
    if (signum == SIGINT) { // handling the sigint signal.
    	...
    	while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
            ...
            shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].status = -1;
            shm->terminatedPQ.processes[++shm->terminatedPQ.tail] = shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head++];
...
void setup_shm(){
    ...
    shm->readyPQ[0].head = 0;
    shm->readyPQ[1].head = 0;
    shm->readyPQ[2].head = 0;
    shm->readyPQ[3].head = 0;
    shm->terminatedPQ.head = 0;
    shm->readyPQ[0].tail = -1;
    shm->readyPQ[1].tail = -1;
    shm->readyPQ[2].tail = -1;
    shm->readyPQ[3].tail = -1;
...
void scheduler_exec(){
    ...
    for (int i = 0; i <NCPU; i++) {
      	sem_wait(&shm->readyPQ_sem);
        if (shm->readyPQ[shm->sched].head+i <= shm->readyPQ[shm->sched].tail && shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].status == 1){
          	sem_post(&shm->readyPQ_sem);
            sem_wait(&shm->readyPQ_sem);
        	shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].status = 0; //running
        	shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].run += TSLICE;
            sem_wait(&shm->time_sem);
        	shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].wait = shm->local_time - shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].arrival - shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].run;
            sem_post(&shm->time_sem);
        	kill(shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].pid, SIGCONT);
            sem_post(&shm->readyPQ_sem);
        }
        else sem_post(&shm->readyPQ_sem);
        usleep(TSLICE * 1000);
        sem_wait(&shm->readyPQ_sem);
        if (shm->readyPQ[shm->sched].head+i <= shm->readyPQ[shm->sched].tail && shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].status == 0) {
          	sem_post(&shm->readyPQ_sem);
          	sem_wait(&shm->readyPQ_sem);
        	kill(shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].pid, SIGSTOP);
        	shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head].status = 1; // waiting
        	Process proc = shm->readyPQ[shm->sched].processes[shm->readyPQ[shm->sched].head++];
        	shm->readyPQ[shm->sched].processes[++shm->readyPQ[shm->sched].tail] = proc;
        	sem_post(&shm->readyPQ_sem);
        }
        else sem_post(&shm->readyPQ_sem);
    }
...
void execute_scheduler_command(char* cmd) {
    ...
    if (pid < 0) {
      perror("Unable to instantiate a child process to run the command");
        ...
        proc->run = 0;
        proc->wait = 0;
        proc->priority = atoi(argv[1])-1;
        sem_wait(&shm->readyPQ_sem);
    	shm->readyPQ[proc->priority].processes[++shm->readyPQ[proc->priority].tail] = *proc;
        sem_post(&shm->readyPQ_sem);
...
int handle_single_command(char* cmd) {
    if (strstr(cmd, "submit") != NULL) {
        ...
        double n = shm->terminatedPQ.tail-shm->terminatedPQ.head+1;
        ...
        printf("\n	Average Completion Time: %f ms\n",avg_completion_time/n);
        printf("Avergae Waiting Time: %f ms\n",avg_wait_time/n);
        printf("\n\nThankyou for using the custom shell...\nCopyrights: Rachit Bhandari & Akshat Lakhera.\n\n");
        return 0;
    }
